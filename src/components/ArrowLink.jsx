...
function ArrowLink({ to = ' ', up = false, className = '', ...props }) {
  return (
    ...
    >
      <Arrow up={up} />
      <span className="bg-darkgray absolute bottom-0 left-0 right-0 top-0 scale-0 rounded-full transition-all group-hover:scale-100"></span>
    </HashLink>
  );
}
...
export function Arrow({ up, block = false, className = '' }) {
  return (
    <svg
      ...
      stroke-darkgray absolute left-2/4 top-2/4 z-50 -translate-x-2/4 -translate-y-2/4 transition-all group-hover:stroke-primary`}
    >
      <path
        strokeLinecap="round"
